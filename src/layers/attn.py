...
class MHAAttn(Attn):
    ...
    def weights_size(self):
        ...
        wq = hidden_size * cfg.num_attention_heads * cfg.head_dim
        wk = hidden_size * cfg.num_key_value_heads * cfg.head_dim
        wv = hidden_size * cfg.num_key_value_heads * cfg.head_dim
        wo = hidden_size * cfg.num_attention_heads * cfg.head_dim
...
class MLAAttn(Attn):
    ...
    def weights_size(self):
        ...
        wq_up = cfg.q_lora_rank * cfg.num_attention_heads * cfg.qk_head_dim
        wq = wq_down + wq_up
        ...
        wkv = wkv_down + wkv_up
        wo = cfg.num_attention_heads * cfg.v_head_dim * hidden_size
...
class LinearAttn(Attn):
    ...
    def weights_size(self):
        ...
        # 投影矩阵权重 s
        wq = hidden_size * cfg.num_key_heads * cfg.key_head_dim
        wk = wq
        wv = hidden_size * cfg.num_value_heads * cfg.value_head_dim
        wz = wv
        wa = hidden_size * cfg.num_value_heads
        wb = wa
        s = wq + wk + wv + wz + wa + wb

        # 卷积权重 wconv
        wconv = cfg.num_key_heads * cfg.key_head_dim * cfg.conv_kernel_dim
        wconv += cfg.num_key_heads * cfg.key_head_dim * cfg.conv_kernel_dim
        wconv += cfg.num_value_heads * cfg.value_head_dim * cfg.conv_kernel_dim
